...
export default async function HostPage({ searchParams }: { searchParams: Promise<{ saved?: string; error?: string }> }) {
  ...
  const assignedScreens = await database.select({ id: screens.id, name: screens.name, playerKey: screens.providerScreenId, venueName: venues.name, city: venues.city }).from(screens).innerJoin(venues, eq(screens.venueId, venues.id)).where(and(eq(venues.hostClerkUserId, user.id), eq(screens.active, true)));
  ...
  return <div className="host-page">
    ...
    {query.error ? <div className="form-error host-alert">Add a headline and message before publishing.</div> : null}
    {assignedScreens.length ? <>
      <HostComposer screens={assignedScreens.map((screen) => ({ id: screen.id, label: `${screen.venueName} — ${screen.name}` }))} />
