...
export async function requireHostUser() {
  ...
  const [actual] = await database.select({ id: appUsers.clerkUserId }).from(appUsers).where(eq(appUsers.clerkUserId, user.id)).limit(1);
  if (!actual) {
    const [invitation] = await database.select({ id: appUsers.clerkUserId }).from(appUsers).where(and(eq(appUsers.email, email), eq(appUsers.status, "invited"))).limit(1);
    if (invitation) {
...
export async function submitHostContent(formData: FormData) {
  ...
  const database = getDatabase();
  const [owned] = await database.select({ screenId: screens.id, venueId: venues.id }).from(screens).innerJoin(venues, eq(screens.venueId, venues.id)).where(and(eq(screens.id, screenId), eq(venues.hostClerkUserId, user.id))).limit(1);
  if (!owned) throw new Error("This screen is not assigned to your host account.");
  await database.insert(hostContent).values({ venueId: owned.venueId, screenId: owned.screenId, submittedByClerkUserId: user.id, status: "scheduled", template, headline, body, callToAction: callToAction || null, startsAt: startsAtRaw ? new Date(startsAtRaw) : new Date(), endsAt: endsAtRaw ? new Date(endsAtRaw) : null });
  revalidatePath("/host");
