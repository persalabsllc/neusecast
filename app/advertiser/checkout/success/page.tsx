...
export default async function CheckoutSuccessPage({ searchParams }: CheckoutSuccessPageProps) {
  ...
  const selectedPlan = getMediaPlan(order.planKey) ?? MEDIA_PLANS.screens;
  const selectedPrice = monthlyCurrency.format(order.amountCents / 100);
  ...
  return (
    <main className="billing-result-page">
      <section className="billing-result-card">
        ...
        <span className="eyebrow">{selectedPlan.name} · {paid ? "Subscription active" : failed || needsAttention ? "Billing needs attention" : "Confirming secure payment"}</span>
        ...
        <p>{paid
          ? `Your ${selectedPlan.name} subscription is active at ${selectedPrice}/month, month-to-month. NeuseCast will review your screen creative today; approved creative is scheduled across every active screen beginning the following broadcast day.${includesRadio ? ` Your underwriting brief is also moving into production for ${selectedPlan.radioAcknowledgmentsPerMonth} Captain 97.1 sponsor acknowledgments per month.` : ""}`
          : failed
            ? `Your ${selectedPlan.name} plan has not started, and no campaign will air until payment is successfully completed. Return to your dashboard to retry or review the campaign.`
            : needsAttention
              ...
              : `Your ${selectedPlan.name} checkout completed, but the signed Stripe confirmation is still processing. This normally takes only a moment; refresh this page or view your campaign status.`}</p>
