...
export default async function ContentPage({ searchParams }: {
  ...
  return (
    <div className="control-page">
      ...
      <section className="panel"><div className="panel-heading"><div><p className="eyebrow">Advertiser campaigns</p><h2>Advertiser creative</h2><p>Paid and house creative share the same scheduling, playback, and proof-of-play pipeline.</p></div></div>{adRows.length ? <div className="content-list">{adRows.map((item) => {
        const houseAd = item.targeting?.houseAd;
        const billingActive = hasActiveBilling(item);
        const billingLabel = houseAd ? "House ad · billing bypassed" : billingActive ? "Billing active" : !item.advertiserActive ? "Account disabled" : item.billingPaused ? "Billing hold" : `Billing ${item.subscriptionStatus.replaceAll("_", " ")}`;
        return <article className="content-row" key={item.id}><span className="metric-icon metric-icon-gold"><Megaphone size={18} /></span><div className="content-main"><div className="content-title-line"><h2>{item.headline || item.name}</h2><span className={`status-badge status-${item.status === "approved" ? "approved" : item.status === "review" ? "pending" : "revision"}`}>{item.status}</span><span className={`status-badge status-${houseAd || billingActive ? "active" : "payment_pending"}`}>{billingLabel}</span></div><p>{houseAd?.sponsor || item.business} · {item.campaign}</p><div className="metadata-row"><span>{item.duration} sec</span><span>{houseAd ? "Control-room house advertisement" : `Campaign ${item.campaignStatus.replaceAll("_", " ")}`}</span><span>Updated {item.updatedAt.toLocaleDateString("en-US", { timeZone: "America/New_York" })}</span></div></div></article>;
      })}</div> : <div className="empty-state"><h3>No advertiser creative yet</h3><p>Paid and house advertisements will appear here.</p></div>}</section>
